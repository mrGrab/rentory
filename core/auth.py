...
def decode_token(token: str, key: str) -> TokenPayload:
    ...
    try:
        payload = jwt.decode(token, key, algorithms=[settings.ACCESS_TOKEN_ALGORITHM])
...
def authenticate_user(session, identifier: str, password: str) -> User | None:
    """
...
class AuthService:
    ...
    def register_new_user(self, session: SessionDep, user_data: UserCreate) -> User:
        """Registers a new user, checking for existing accounts first."""
        existing_user = get_user_by_username(
            session, user_data.username
        ) or get_user_by_email(session, user_data.email)
    ...
    async def handle_google_callback(
        ...
        try:
            ...
            if not user_info or not user_info.get("email_verified"):
                raise AuthenticationException("Could not verify Google account.")
