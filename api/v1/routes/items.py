...
from models.item import Item, ItemCreate, ItemFilters, ItemPublic, ItemUpdate
from models.item_variant import ItemVariant, ItemVariantPublicInternal
...
def get_item_or_404(
    item_id: UUID, service: Annotated[ItemService, Depends(get_item_service)]
) -> Item:
...
def to_public(item: Item, availability: dict[UUID, int] | None = None) -> ItemPublic:
    """Convert Item to ItemPublic with variant and order information.

    `availability` optionally maps variant id to available_quantity, since
    that value is only computed by the availability-check endpoint.
    """
    order_ids = set()
    public_variants = []
    ...
    for variant in item.variants:
        if not variant.is_archived:
            ...
            if availability is not None:
                update["available_quantity"] = availability.get(variant.id)
            public_variants.append(
                ItemVariantPublicInternal.model_validate(variant, update=update)
            )
        for link in variant.order_links:
            order_ids.add(link.order_id)
    return ItemPublic.model_validate(
        item, update={"order_ids": list(order_ids), "variants": public_variants}
    )


# ---------- Dropdown Endpoints ----------
@router.get("/categories", response_model=list[str], summary="Get category options")
def get_categories(
...
@router.get("/statuses", response_model=list[str], summary="Get status options")
def get_statuses(
    current_user: CurrentUser,
    service: Annotated[ItemService, Depends(get_item_service)],
):
...
@router.get(
    "/variant-statuses", response_model=list[str], summary="Get variant status options"
)
def get_variant_statuses(
    current_user: CurrentUser,
    service: Annotated[ItemService, Depends(get_item_service)],
):
    """Get distinct variant status values for dropdown"""
    logger.debug(f"User {current_user.username} fetching variant status options")
...
def list_items(
    ...
    logger.info(f"User {current_user.username} retrieved {len(result)}/{total} items")
    return result
...
def create_item(
    ...
    logger.info(f"User {current_user.username} creating item '{item_in.title}'")
...
def get_item(
    current_user: CurrentUser, item: Annotated[Item, Depends(get_item_or_404)]
):
...
def check_availability(
    ...
    if start_time and end_time:
        item, availability = service.check_availability(
            item, start_time, end_time, exclude_order_id
        )

    logger.info(f"Availability checked for item {item.id}")
    return to_public(item, availability)
